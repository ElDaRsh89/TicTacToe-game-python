    ...
    while yes:
        ...
        while game:
            ...
            while not cycle_finished:
                print ("         Tic Tac Toe           ")
                print("First", new_board)  # The first board that will appear to the user. Of course an empty one. (line 88)
                # 1) the user input will be validated by the validator funcion and the correct input will be stored in
                # the choice variable.
                ...
                while choice not in positions_list:
                    ...
                    # 3) After choice is OK. put the choice on the empty board and store the new board in the same variable.
                    ...
                    # the cycle will break skipping the computer choice. when the while loop checks "game",
                    # it will find it "False" and will not enter into a new game.
                    if win(new_board):
                        print("You Won")
                        game = False
                        cycle_finished = True
                        ...
                        if not cycle_finished:
                            ...
                            # choice
                            # 5) Check if the user picked the last cell in the board which corresponds to an empty positions
                            # list. If so, then it is a tie. If this choice was a winning pattern, the game would have ended
                            # on line 115.
                            if len(positions_list) == 0:
                                ...
                                # Firstly : The program will loop over each position left and put an "O" in that position.
                                for position in positions_list:
                                    fict_ai_board_two = put_symbol_on_board(position, "O", new_board)
                                    # If the resulting board has a winning pattern, the board will be updated like that
                                    # and the game will finish as the computer has won. else (line 142), continue to the
                                    # next position.
                                    if win(fict_ai_board_two):
                                        ...
                                        continue
                                if not game:  # After finishing looping over all positions, check if game is still True. if not
                                    # break the cycle and the game is finished.
                                    break
                                # Secondly: The program will loop again over each position but this time it will put an "X"
                                # in the position and see if the user will win if the user put "X" in the next cycle.
                                ...
                                for position in positions_list:
                                    ...
                                    if win(fict_ai_board):  # if this position will make the user win
                                        ...
                                        continue
                                # After the 2 loops finished and still no "O' was put (cycle_finished still False)
                                # then choose a random position from positions list and put "O" in it.
                                if not cycle_finished:
